...
const ReceiverSigninScreen = (props) =>{

  const [username,setUsername] = useState('');
  const [password,setPassword] = useState('')

  const sendCred = async (props) =>{
    fetch("http://192.168.1.6:3001/signin",{
      method:"POST",
      headers: {
  ...
  return(
    ...
    <TextInput
      ...
      <Text style={{left:190, top:200, fontSize:18, color:'dodgerblue', fontStyle:'italic'}} onPress={()=>props.navigation.navigate("ReceiverSignup")}>
        Register Now!
      </Text>
    </View>
...
export default ReceiverSigninScreen;